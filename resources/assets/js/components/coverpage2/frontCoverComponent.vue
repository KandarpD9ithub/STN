    ...
    <div class="col-sm-8 col-md-8 col-xs-12 right_side">
        <div class="padding_left_right">
            ...
            <form>
                ...
                <div class="form-group">
                    <input type="text" data-vv-name="title" maxlength="35" v-model="title" :class="{'form-control': true, 'alert-danger': errors.has('title'), 'step_input': true }" placeholder="For example : ABC Trvel - John Smith">
                    <!-- <span v-show="errors.has('title')" class="color-red">{{ errors.first('title') }}</span> -->
                </div>
                <div class="form-group">
                    <input type="text" v-model="subTitle"  maxlength="25" class="form-control" placeholder="Member / Affiliate Name">
                </div>
                <div class="form-group">
                    <div class="row">
                        <div class="col-xs-9 col-md-9 col-sm-9 pull-left ">
                            <div class="full_img magin_bottom_70 step1_bg_img book_2_fcbg">
                                <div class="above_step1_text">
                                    <h4><img v-if="includeLogo" :src="coverLogo" alt="" title="">{{ !includeLogo ? title : '' }}<br/>{{ !includeLogo ? subTitle : '' }}</h4>
                                </div>
                            </div>
                        </div>
                        <div class="col-xs-3 col-md-3 col-sm-3 approve_btn pull-right">
                            ...
                            <button type="button" class="green_btn submit_btn" style="margin-right: 0px;" @click="saveBook(1)"><span>Save & Preview</span></button>                            
                    ...
                    if(result) {
                        ...
                        .then(response => {
                            ...
                            if (type === 2) {
                                ...
                                this.$router.push('/cover2/insideFrontCover')    
                            } else {
                                this.$store.state.successMessage.push({message: 'Front Cover Saved!'})
                                // Open preview
                                var win = window.open(this.$url + 'preview/book2' + '/1/' + this.$session.get('userId') , '_blank');
                                win.focus();
                            }
